...
- [Build the React Client (Optional)](#build-the-react-client-optional)
- [Dockerize](#dockerize)
...
```json
{
  "Date": "Sat, 03 Feb 2024 07:24:11 GMT",
...
Add a proxy to `package.json` to set the port that the frontend (client) queries to make backend (API) requests. The React default port is 3000 and will be directed to the port indicated in the proxy for backend/API requests. Below, I set the proxy port to 5001. If you leave the backend (above) at the default port of 5000 then change the port in the proxy line to 5000.
